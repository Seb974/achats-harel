...
class OdooApiService
{
    ...
    public function createRFQ(int $supplierId, array $lines, array $options = []): array
    {
        ...
        // Mark RFQ as locked by the app
        try {
            $this->write('purchase.order', [$orderId], ['x_locked_by_app' => true]);
        } catch (\Throwable $e) {
            $this->logger->warning('Could not set x_locked_by_app', ['error' => $e->getMessage()]);
        }
    ...
    public function createPurchaseOrder(int $supplierId, array $lines, array $options = []): array
    {
        ...
        // Retrieve the reception picking generated by confirmation
        $pickingId = null;
        $pickingName = null;
        try {
            ...
            if ($orderName) {
                $pickings = $this->searchRead(
                    'stock.picking',
                    [['origin', '=', $orderName], ['picking_type_id.code', '=', 'incoming']],
                    ['id', 'name', 'state'],
                    1
                );
                if (!empty($pickings)) {
            ...
            $this->logger->warning('Could not retrieve reception picking', ['error' => $e->getMessage()]);
        }
